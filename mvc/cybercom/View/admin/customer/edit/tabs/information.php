...
<div class="form-row">  
    ...
    <div class="col-lg-4">
        ...
        <select class="form-control" name="customer[groupId]" id="status">
        <?php foreach ($groupName->data as $key=>$value): ?>
        <option value="<?php echo $value->groupId;?>" <?php if($value->groupId == $customer->groupId) echo "selected";?> ><?php echo $value->groupName;?></option>
        <?php endforeach;?>
    </select>
    </div>
    <div class="col-lg-4">
...
<div class="form-row">
    ...
    <select class="form-control" name="customer[status]" id="status">
        ...
        <option value="<?php echo $key;?>" name="<?php echo $key;?>" <?php if($customer->status == $value) echo "selected";?> ><?php echo $value;?></option>
        <?php endforeach;?>
    </select>
    </div>
    <div class="col-lg-2"></div>
